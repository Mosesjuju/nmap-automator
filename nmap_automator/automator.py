# Main Nmap Automator logic
# Move code from core/nmap_automator_optimized.py here
...
def main():
    ...
    print(Fore.CYAN + Style.BRIGHT + "#      NMAP AUTOMATOR v{} - ENHANCED EDITION      #".format(__version__))
    print(Fore.CYAN + Style.BRIGHT + "# Modern, robust, and extensible network scanner  #")
    print(Fore.CYAN + Style.BRIGHT + "#  Author: mosesjuju                             #")
    ...
    for target in args.targets:
        ...
        if args.masscan and check_masscan_available():
            ...
            print(Fore.GREEN + f"[Tool Chaining] Masscan completed. Ports discovered: {port_string}" + Style.RESET_ALL)
        else:
        ...
        print(Fore.GREEN + f"[Tool Chaining] Running Nmap scan on ports: {port_string}" + Style.RESET_ALL)
...
<head>
    ...
    <style>
        body {{ font-family: 'Segoe UI', Arial, sans-serif; background: #f8f9fa; color: #222; margin: 0; padding: 2em; }}
        h2 {{ color: #0077cc; }}
...
<body>
    <h2>NMAP Scan Results for {target}</h2>
    <div class="summary">
        <strong>Date:</strong> {date}<br>
        <strong>Command:</strong> <code>{cmdline}</code>
    </div>
'''.format(target=target, date=datetime.now().strftime('%Y-%m-%d %H:%M:%S'), cmdline=' '.join(cmd))
            # Try to render findings as a table if possible
            def render_table(findings):
                if isinstance(findings, list) and findings and isinstance(findings[0], dict):
                    keys = findings[0].keys()
                    table = '<table><tr>' + ''.join(f'<th>{k}</th>' for k in keys) + '</tr>'
                    for row in findings:
                        table += '<tr>' + ''.join(f'<td>{row.get(k, "")}</td>' for k in keys) + '</tr>'
                    table += '</table>'
                    return table
                return None

            if findings:
                table_html = render_table(findings)
                if table_html:
            ...
            html_content += '''<div class="footer">Generated by NMAP Automator v{version}</div></body></html>'''.format(version=__version__)
